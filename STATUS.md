...
最終更新: 2026-06-30（①キャラ名 ひなた → ひより 巻き戻し ②TTS エンジン切替 UI 削除 → Aivis 固定化 ③設定画面で呼び名 `users.name` を編集可能化。サービス名は「みさき×ひより なかよしtalk」(略：みさひな)、本番ドメインは misahina.com 据え置き。§11 の 2026-06-30 エントリ参照）
...
├── prompts/
│   ├── characters/
│   │   ├── misaki.md          案内役 みさき・キャラ設定 (Aivis model e9339137...)
│   │   └── hiyori.md          盛り上げ役 ひより・キャラ設定 (Aivis model a670e6b8...)
│   └── kaiwa/
...
└── src/
    ...
    │       ├── me/name/route.ts         呼び名 (`users.name`) の GET / PUT（最大8文字 / ひらがな推奨）
    │       └── tts/route.ts             Aivis Cloud で合成（送信直前に tts-readings の置換を適用）
    ├── components/
    │   ├── MapView.tsx          Leaflet + 地理院タイル(pale) + 現在地 circleMarker
    │   ├── SpeechRow.tsx        キャラ画像 + セリフバブル
    │   ├── SettingsOverlay.tsx  地図右上の歯車ボタン+設定ポップアップ(呼び名編集/サブスク管理/ログアウト)
    │   └── SwRegister.tsx       Service Worker 登録
    └── lib/
        ├── db.ts              mysql2 connection pool
        ├── session.ts         iron-session 設定
        ├── characters.ts      みさき/ひより の定義 (aivisModelUuid, promptPath)
...
（2026-06-30: ElevenLabs TTS は UI 切替の廃止に伴い未使用化。`.env.local` `ELEVENLABS_API_KEY` は残置しているが削除可。aicyc 側で引き続き使用中）
...
### 2026-06-30: 設定画面で呼び名（`users.name`）を編集可能に

LLM プロンプトの `{user_name}` placeholder に流し込まれる呼び名を、ユーザー自身がアプリ内で変更できるようにした。これまでは account.misahina.com 側でしか変更導線がなかった。

**変更点**:
- `src/app/api/me/name/route.ts` 新規:
  - `GET /api/me/name` → `{ name: string | null }`
  - `PUT /api/me/name` `{ name: string }` → trim → 1〜8 文字 / 制御文字不可 を検証して `UPDATE users SET name = ? WHERE id = ?`
- `src/components/SettingsOverlay.tsx`:
  - 設定ポップアップを開くと `/api/me/name` GET で現在値をロード
  - 8文字 maxLength の `<input>` + 「呼び名を保存」ボタン。文字数カウンタと保存結果メッセージ（成功 / エラー）
  - 「呼び名」セクションを「サブスクリプション管理」「LOGOUT」より上に配置
  - ボタンは `空 / 8文字超 / 未変更 / 保存中` で disable
- 制約: 最大 8 文字、ひらがな推奨（強制はしないので漢字・カナ・英数字も可）

**反映タイミング**:
- LLM 会話の `{user_name}` は `/api/generate` が毎ターン `SELECT name FROM users` するため、保存後の**次ターンから即時反映**
- JWT cookie 内の `name` 旧値は次回ログインで更新される（tabikoto は JWT の `name` を fallback として使うが、DB を必ず先に読むので影響なし）
- `users` テーブルは account.misahina.com と共有のため、Account 画面側の表示にも反映される

**ステータス**:
- ✅ tsc / 本番ビルド OK
- ⏳ 本番反映（再起動）

### 2026-06-30: TTS エンジン切替 UI を削除し Aivis 固定化

設定アイコン（地図右上の歯車）からの `Aivis / ElevenLabs` 切替ボタンを削除。常時 Aivis Cloud に固定。

**変更点**:
- `src/components/SettingsOverlay.tsx`: TTS セクション（ラベル + 2ボタン + 補足文）削除。`useEffect` / `engine` state / `handleEngineChange` も削除
- `src/lib/client/tts.ts`: `/api/tts` POST から `engine` フィールドを削除。`getTtsEngine` import 削除
- `src/lib/client/settings.ts`: ファイル削除（`git rm`）。この副作用で localStorage `hinavi.ttsEngine` キーはユーザー端末上に残るが、次回読み出さなくなるので影響なし
- `src/app/api/tts/route.ts`: `engine` パラメータと `synthesizeElevenLabs()` 関数、`TtsEngine` 型、`ELEVENLABS_BASE` 定数を削除。常に `synthesizeAivis()` を呼ぶ
- `src/lib/characters.ts`: `Character.elevenLabsVoiceId` フィールドと CHARACTERS 内の voiceId を削除

**残置**:
- `.env.local` `ELEVENLABS_API_KEY`: 削除可だが、aicyc 側で同キーを共有している可能性があるため据え置き
- ElevenLabs Pro プラン契約自体: 解約判断はユーザー側で

**ステータス**:
- ✅ tsc / 本番ビルド OK
- ⏳ 本番反映（再起動）

### 2026-06-30: キャラクター名 ひなた → ひより 巻き戻し（misahina.com は据え置き）

前日 (2026-06-29) に `hiyori` → `hinata` リネームを行ったが、その後 **X 上で既に「みさき×ひより」名義でプロモーションが開始** されていたことが判明したため、表示名/識別子を `ひより` に **巻き戻し**。

サービス名と本番ドメインの整合は以下の解釈で整理:
- サービス名: **「みさき×ひより なかよしtalk」**
- 略称: **「みさ」「ひ」「な」** = **みさひな** (`み`さき + `ひ`より + `な`かよし)
- 本番主ドメイン: `misahina.com`（**据え置き**）

**変更点（2026-06-29 リネームの逆操作）**:
- コード識別子: `hinata` → `hiyori`（`apps/tabikoto/src/lib/characters.ts`, `types.ts`, `client/conversationLoop.ts`, `app/api/generate/route.ts`, `app/page.tsx`, `public/sw.js`）
- 表示名: `ひなた` → `ひより`（`prompts/characters/hiyori.md`, `misaki.md`, `prompts/kaiwa/kaiwa{1,2,3}.md`）
- ファイルリネーム: `public/characters/hinata.png` → `hiyori.png`, `prompts/characters/hinata.md` → `hiyori.md`（`git mv`）
- DB マイグレーション: `sql/migrations/20260630_revert_hinata_to_hiyori.sql`（`UPDATE conversations SET speaker='hiyori' WHERE speaker='hinata'`、実行済：misaki 36 / hiyori 36）
- `public/sw.js` `CACHE_NAME` を `hinavi-v8` → `hinavi-v9` にバンプ（プリキャッシュ URL が `/characters/hinata.png` → `/characters/hiyori.png` に変わったため）
- 音声設定（Aivis `a670e6b8...` / ElevenLabs `OSwaPSNdfituxkWcjlkR`）は**据え置き**。声・性格設定はそのまま、名前だけ戻す

**残作業**:
- 本番デプロイ（`npm run start:tabikoto` 再起動）
- PWA インストール済端末は SW の `hinavi-v9` activate で旧キャラ画像キャッシュが破棄される

### 2026-06-29: 課金・コトポシステム実装 + Stripe サンドボックス動作確認完了

- 4 プラン構成: ちょこっと ¥100 / ライト ¥780/月 / スタンダード ¥2,480/月 / プロ ¥4,980/月
- Stripe Checkout (初回購入) + Stripe Customer Portal (解約/プラン変更/領収書) のハイブリッド構成 (案A 採用)
- アップグレード即時+日割り、ダウングレード期間末で適用 (Portal 設定で実現)
- アクティブサブスク検出 + 重複契約防止 + UI 動的分岐
- サンドボックスで一通りのフロー動作確認: 初回お試し付与、各プラン購入、解約予約、撤回、アップ/ダウングレード、FIFO 消費すべて正常
- バグ修正: invoice line items の最大金額抽出 (アップグレード時の credit/charge 混在対策)、Webhook 認証バイパス (proxy.ts)、Stripe API dahlia 構造変更対応
- 詳細: §14 を参照

### 2026-06-29: キャラクター名 ひより → ひなた リネーム

`misahina.com`（みさき×ひなた）のドメイン名に実装側を合わせるためのリネーム。当初 2 人目キャラを「ひなた」にする予定がコード上は「ひより」で実走していた齟齬を解消。

- コード識別子: `hiyori` → `hinata`（`apps/tabikoto/src/lib/characters.ts`, `types.ts`, `client/conversationLoop.ts`, `app/api/generate/route.ts`, `app/page.tsx`, `public/sw.js`）
- 表示名: `ひより` → `ひなた`（`prompts/characters/hinata.md`, `misaki.md`, `prompts/kaiwa/kaiwa{1,2,3}.md`, `characters.ts` の `displayName`）
- ファイルリネーム: `public/characters/hiyori.png` → `hinata.png`, `prompts/characters/hiyori.md` → `hinata.md`（`git mv`）
- DB マイグレーション: `sql/migrations/20260629_rename_hiyori_to_hinata.sql`（`UPDATE conversations SET speaker='hinata' WHERE speaker='hiyori'`）
- 音声設定（Aivis / ElevenLabs の UUID/VoiceId）は据え置き。声はそのまま、名前だけ差し替え
...
### 2026-06-19: 画像素材差し替え（みさき / ひなた / PWA アイコン）

新キャラ画像は縦長（520×600）、PWA アイコンは 512×512。素材は `/home/ec2-user/sozai/` から `public/` 配下にコピー。

**変更点**:
- `public/characters/misaki.png`, `public/characters/hinata.png` を新素材で上書き（旧 500×417 横長 → 新 520×600 縦長）
...
- `prompts/characters/{misaki,hinata}.md` / `prompts/kaiwa/kaiwa1.md` / `prompts/kaiwa/kaiwa2.md` / `prompts/kaiwa/kaiwa3.md` のシーン指示は「旅行中」前提に書き換え済（ユーザー手動編集）

**運用上のメモ**:
- topics は SQL 直接編集で増減可能。`UPDATE topics SET is_active = 0 WHERE id = ?` で論理削除
- 1回の rest ターン = 2発話（みさき/ひなた）= 1話題で完結する想定。連続 rest ターンは無いので「同じ話題が続く」現象は基本起きない
...
### 2026-06-11: ひなたの Aivis モデル UUID を差し替え

ひなたの音声が安定しない（推定: 抑揚や子音の暴れ）所見を受け、Aivis Cloud 上の別モデルへ差し替え。

- `src/lib/characters.ts:25` `hinata.aivisModelUuid`
...
- `kaiwa1.md` / `kaiwa2.md` / `kaiwa3.md` の会話順指示を「みさきは話題の後にひなたへ話しかける」→「みさきとひなたがお互いに話しかけ合う」に変更（2拍子の単調感を緩和）
...
- 呼称が分からないと AI は呼びかけを諦める挙動。`misaki.md` / `hinata.md` の「ユーザーへの呼びかけ：{user_name}さん」と `kaiwa3.md` の時刻読み上げ例文に placeholder を復活させて解消
...
**Phase B — 会話コア再構築**
- プロンプト構造を 2 層化: `prompts/characters/{misaki,hinata}.md`（共通キャラ設定） + `prompts/kaiwa/kaiwa{1,2,3}.md`（シーン別指示）。旧 `{misaki,hinata}{1,2}.md` 4 ファイルを削除
- 会話ループを 3 モード化: `turnNo % 30` 時間 > `turnNo % 6` 休憩 > その他スポット
- `/api/generate` を 2話者 1コール化。`responseSchema` で `{misaki, hinata}` JSON 出力を強制。HTTP / JSON パース失敗時は 1 回リトライ
...
- `conversations` テーブルに `mode` カラム追加。同一ターンの 2 行（misaki, hinata）にモードが入る
...
**実機確認状況**
- ✅ ビルド + 本番反映完了
- ✅ Gemini 3.1 Flash Lite + responseSchema の JSON 出力動作確認（直接コールで `{misaki, hinata}` 取得）
...
- `src/lib/characters.ts`: `voicevoxSpeakerId` を削除し `aivisModelUuid` に置換
  ...
  - ひなた: `734c12b6-eaf2-4dbd-8596-8663c72d2afa` ← 2026-06-11 に `a670e6b8-0852-45b2-8704-1bc9862f2fe6` へ差し替え（音声安定性の改善目的）
...
1. **ElevenLabs TTS 対応**
   ...
   - `lib/characters.ts` に `elevenLabsVoiceId` を追加（みさき=`ugYcuAusTuWCSOpJD0Xd` / ひなた=`OSwaPSNdfituxkWcjlkR`）
...
3. **奇数/偶数ターンでプロンプト構造を切替**
   - `prompts/characters/{misaki,hinata}1.md` / `{misaki,hinata}2.md` の2系統に分割
...
- 奇数/偶数の切替自体は意図通り効いている。ただし2拍子サイクルで単調になりがちと観察（みさき紹介→ひなた反応→みさき継続→ひなた継続 の繰り返し感）
...
ターン定義: みさき・ひなたが1回ずつ発言する1往復 = 1ターン。
...
現行の `{misaki,hinata}{1,2}.md` 4ファイル構成を廃止し、キャラ設定とシーン指示を2層化:

```
prompts/
├── characters/
│   ├── misaki.md          みさきのキャラ設定（共通・口調・性格・呼称ルール）
│   └── hinata.md          ひなたのキャラ設定（共通）
└── kaiwa/
    ├── kaiwa1.md          スポットモード用シーン指示
...
`/api/generate` 側で `[misaki.md, hinata.md, kaiwa<N>.md]` を結合してプロンプト生成。キャラ設定変更時に1箇所修正で済むようになる。

### 12.4 2話者1コール化（Gemini 呼出回数半減）

ユーザー割り込みが無い前提を活かして、みさき発話＋ひなた回答を **1コールで JSON 出力** に変更。

- レスポンス: `{misaki: string, hinata: string}`
- Gemini `responseSchema`（Structured Output）で JSON 形式を強制
- `conversationLoop.ts` のフロー: `generatePair(mode, turnNo, ...)` → みさき再生 → 10秒待ち → ひなた再生 → 10秒待ち
...
- 同時に **無料デモサイト（freetalk = みさき×ひなたのフリートーク, 別開発者・別リポジトリ）→ 旅コト本体** の動線を作るため、サブドメイン間で SSO を共有する
...
## 14. 課金・コトポ (ポイント) 導入 [Phase1+2 実装＋動作確認完了 2026-06-29]

### 14.1 ビジネス要件

- **時間制限ではなくポイント (コトポ) 制**: 電波状況などで利用できなかった時間の判定が難しいため、AI 会話 1 回生成あたりのポイント消費にする
- **プラン**:
  - **ちょこっとプラン**: 100円 (税込) / 2,000 コトポ (想定 約3時間) — 都度購入 ※「初めての人のお試し」位置付け
  - **ライトプラン**: 月額 780円 (税込) / 10,000 コトポ (想定 約15時間) — サブスク
  - **スタンダードプラン**: 月額 2,480円 (税込) / 40,000 コトポ (想定 約60時間) — サブスク
  - **プロプラン**: 月額 4,980円 (税込) / 100,000 コトポ (想定 約150時間) — サブスク
...
- **プラン変更**: アップグレード=即時+日割り請求、ダウングレード=期間末で適用
- **将来**: ちょこっとプラン無料化キャンペーン (1 回限り) / 招待コード (双方付与) / サブスク契約者向けの大盛り追加チャージパック

### 14.2 設計決定

| 論点 | 決定 |
|---|---|
| サブスク月次付与タイミング | Stripe 契約日基準。契約月は即時、翌月以降は契約日 (無ければ末日) |
...
| サブスク管理 UI | **Stripe カスタマーポータルに任せる（案A）**: 解約・支払い方法変更・領収書・プラン変更すべて Portal で完結 |
| プラン変更 | アップグレード=即時+比例配分、ダウングレード=期間末で適用+比例配分なし (Portal 設定で実現) |
| 重複契約防止 | アクティブなサブスクが存在する場合、サーバ側で 409 を返却。UI は購入ボタンを Portal 誘導に切り替え |
| 投資なしプラン枠 | `config.ts` の `visible: false` で予約可能。Stripe で商品登録後に Price ID 投入＋ `visible: true` に切替で公開 |
...
- `subscriptions` — Stripe Subscription のミラー (stripe_subscription_id UNIQUE, status, period_start/end, cancel_at, canceled_at)

**サーバロジック**:
- `src/lib/billing/config.ts` — プラン定義 (`PLANS` 4プラン)・閾値・`calcExpiresAt(date)` (翌々月末を JST で計算)・`findPlanByPriceId(priceId)` (Webhook で逆引き)・`visiblePlans()` (UI 表示順)・`isSubscriptionPlan(plan)`
...
- `src/lib/billing/subscriptions.ts` — `upsertSubscriptionFromStripe` / `findUserIdBySubscription` / `findActiveSubscriptionByCustomer` / `findActiveSubscriptionByUser` / `findStripeCustomerIdByUser`
...
- `POST /api/billing/checkout` — `{plan: PlanKey}` で Stripe Checkout Session を作成し URL 返却。**サブスクの場合は既存アクティブサブスクを検出して 409 で重複防止**
- `GET /api/billing/subscription` — 現在のアクティブサブスク情報 (planKey/cancelPending 等) を返却。UI 分岐用
- `POST /api/billing/portal` — Stripe カスタマーポータルセッションを作成し URL 返却 (解約/プラン変更/領収書取得すべて Portal で完結)
- `POST /api/webhooks/stripe` — 署名検証 → `checkout.session.completed` / `invoice.paid` / `customer.subscription.*` を処理
  - **アップグレード対応**: invoice の line items から最大金額のラインの price_id を抽出し `findPlanByPriceId` で逆引き (旧プラン credit と新プラン charge の混在対策)
  - **冪等性**: 同じ stripe_ref (invoice.id or payment_intent.id) で重複付与しない
  - **subscription 関連付け**: 新フィールド `invoice.parent.subscription_details.subscription` → customer ID 逆引き → Stripe API 取得の三段フォールバック

**ミドルウェア** (`src/proxy.ts`):
- Stripe Webhook (`/api/webhooks/*`) は認証バイパス。それ以外は JWT 認証必須

**UI**:
- `src/components/PointsBadge.tsx` — 残高バッジ (右上、設定アイコンの左隣)
- `src/components/LowBalanceBanner.tsx` — 残 100 未満の警告バナー
- `src/components/PurchaseModal.tsx` — 4 プラン購入導線。**アクティブサブスク検出して動的表示分岐**
  - 契約中プラン: 「ご契約中」バッジ + 緑枠 → クリックで Portal 起動
  - 他のサブスクプラン: 「プラン変更は管理画面から」 → 同じく Portal 起動
  - ちょこっと (one_time): 常時購入可能
- `src/components/SettingsOverlay.tsx` — 「サブスクリプション管理」ボタン追加 → `/api/billing/portal` 経由で Portal 起動
- `src/app/page.tsx` — 残高ポーリング (30 秒) + バッジ + バナー + 枯渇時自動モーダル
- `src/app/billing/success/page.tsx` / `cancel/page.tsx` — Stripe Checkout のリダイレクト先

**クライアントヘルパ** (`src/lib/client/points.ts`):
- `fetchBalance()` / `startCheckout(plan)` (409 ハンドリング含む) / `fetchActiveSubscription()` / `openCustomerPortal()` / `InsufficientPointsError`

**会話ループ** (`src/lib/client/conversationLoop.ts`):
- 402 (insufficient_points) を `InsufficientPointsError` で捕捉 → `onInsufficientPoints` コールバックで UI 通知
- `onPointsConsumed` コールバックで成功時に残高再フェッチ

**バッチ**:
- `apps/tabikoto/scripts/expire-lots.mjs` — `point_lots.expired=1` を日次バッチで更新 (cron 設定例コメントあり)

### 14.4 サンドボックスでの動作確認結果 [2026-06-29]

| シナリオ | 結果 |
|---|---|
| 初回お試し付与 (1ユーザー1回限り、100コトポ) | ✅ 正常 |
| ちょこっとプラン購入 (Stripe Checkout 都度課金) | ✅ 正常、2,000 付与 |
| ライトプラン購入 (Stripe Checkout サブスク初回) | ✅ 正常、10,000 付与 |
| FIFO 消費 (initial_trial → chokotto → light の順) | ✅ 正常 |
| ライトプラン解約予約 (Portal) | ✅ subscriptions.cancel_at に日時保存 |
| 解約予約の撤回 (Portal の「サブスクリプションをキャンセルしない」) | ✅ 正常 |
| ライト → スタンダードアップグレード (Portal、日割り即時請求) | ✅ 修正後正常 (初回は invoice の line[0] 取得バグで Light 付与のみ → 最大金額ライン抽出に修正) |
| スタンダード → プロアップグレード | ✅ 100,000 正常付与 |
| 重複サブスク防止 (アクティブ中のサブスク Checkout) | ✅ 409 で防止、UI は Portal 誘導 |

### 14.5 残作業 (本番化前)

**Stripe サンドボックス設定 (完了)**:
- [x] Product 4 つ作成（ちょこっと/ライト/スタンダード/プロ）
- [x] Stripe CLI で Webhook 転送設定
- [x] `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` / `STRIPE_PRICE_ID_*` (4種) を `.env.local` に設定
- [x] DB マイグレーション適用
- [x] カスタマーポータル設定（プラン切替・解約・支払い方法変更・領収書 すべて有効化）
  - 日割り計算: ON
  - 請求タイミング: 更新時点で直ちに日割り請求
  - ダウングレード: 請求期間の終了時まで待ってから更新

**本番化時に必要な対応**:
- [ ] Stripe 本番アカウント有効化（事業者情報・銀行口座・本人確認）
- [ ] 本番モードで Product/Price 4 つ再作成 → live モードの `STRIPE_PRICE_ID_*` を本番 `.env` に
- [ ] 本番 Webhook endpoint 登録（`https://tabikoto.hinavi.mediowl.ai/api/webhooks/stripe`） → live モードの `whsec_...`
- [ ] 本番モードでも同じ Portal 設定（テスト/本番モードはデータ分離）
- [ ] cron 設定: `0 19 * * * set -a; . .env.local; set +a; node scripts/expire-lots.mjs >> /var/log/hinavi/expire-lots.log 2>&1`
- [ ] 特商法表記ページ（法律上必須）
- [ ] 利用規約 — コトポの有効期限、解約時の扱い、返金不可などを明記
- [ ] ALB の WebSocket 通過設定 (Next.js HMR 用、開発時のみ気になる、本番影響なし)

**運用後の検討事項**:
- [ ] 実測してレート調整 — 10 コトポ/往復 は仮置き、テスト走らせて再検討
- [ ] サブスク契約者向け追加チャージパック — ちょこっと (お試し用) とは別に、`topup_small/medium/large` 等を追加。Stripe で商品登録 → `PLANS` に追加 → サブスク契約中は対象を出し分け
- [ ] **ちょこっとプランの位置付け整理** — 「初めての人のお試し」用なので、サブスク契約中ユーザーへは非表示 or 「お試し」明示にする UX 検討
- [ ] 招待コード — `invite_codes` テーブル設計、コード発行 UI、コード利用時の双方付与
- [ ] ちょこっと無料化キャンペーン — `campaign_chokotto_free` 付与 (1ユーザー1回限り) の発動条件設計

### 14.6 環境変数 (`.env.local`)

```
STRIPE_SECRET_KEY=sk_live_...        # or sk_test_... in dev (sandbox)
STRIPE_WEBHOOK_SECRET=whsec_...
STRIPE_PRICE_ID_CHOKOTTO=price_...   # ちょこっとプラン (one_time)
STRIPE_PRICE_ID_LIGHT=price_...      # ライトプラン (recurring monthly)
STRIPE_PRICE_ID_STANDARD=price_...   # スタンダードプラン (recurring monthly)
STRIPE_PRICE_ID_PRO=price_...        # プロプラン (recurring monthly)
...
- **Next.js dev サーバの allowedDevOrigins**: 開発機を `tabikoto.hinavi.mediowl.ai` ドメイン経由で叩くと cross-origin として弾かれる → `next.config.ts` の `allowedDevOrigins: ['tabikoto.hinavi.mediowl.ai']` で許可
- **proxy.ts (Next 16 middleware) が Webhook を弾く**: 全リクエストが認証必須ミドルウェアを通るため、Stripe Webhook が 307 redirect される → `/api/webhooks/` を public パスに追加
- **Stripe CLI の WebSocket 切断**: 長時間アイドルで `websocket: close sent` ログとともに切断、その間 Webhook はローカルに届かない → 再起動 + `stripe events resend <evt_id>` で復旧。本番は直接 Webhook endpoint なので無関係
- **新 Stripe API (2026-06-24.dahlia) の Invoice 構造変更**: `invoice.subscription` が廃止、`invoice.parent.subscription_details.subscription` に移動。三段フォールバック (parent 経由 → customer ID 逆引き → Stripe API 取得) で対応
- **アップグレード時の invoice line items**: 旧プラン credit (負額) と新プラン charge (正額) の複数ライン構造になる。line[0] を見ると古いプランを拾うバグ。**最大金額のラインを選ぶ**ロジックで解決
- **Stripe サンドボックスの戻るリンク**: Portal の左上戻るボタンが「サンドボックスに戻る」表記で紛らわしいが、`return_url` に正しく飛ぶ。本番モードではビジネス名が表示される
